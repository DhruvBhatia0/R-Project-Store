...
## Now we set aside the future data (we would only need that later when we make forecasts)
## And focus on the training data

## * 4.1 Panel Data Splitting ----

## Split the dataset into analysis/assessment sets
...
recipe_spec_1 <- recipe(Weekly_Sales ~ ., training(splits)) %>% 
  step_timeseries_signature(Date) %>%  # Break 'Date' into different calendar features
  step_rm(matches("(.iso$)|(.xts$)|(day)|(hour)|(minute)|(second)|(am.pm)")) %>%  # Remove unnecessary features
  step_normalize(Date_index.num, Date_year) %>%  # Normalize selected variables
  step_zv(all_predictors()) %>%  # Remove features with zero variance
  step_mutate(Date_week = factor(Date_week, ordered = TRUE)) %>%  # Convert 'Date_week' into a factor
